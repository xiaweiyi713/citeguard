...
48 条 evidence-level cases 加上 6 条 citation-set policy cases。它用于回归测试
...
- abstention rate
- false-support rate
- support overcall count / rate (`support_overcall_count` /
  `support_overcall_rate`), counting both `supported` and `weakly_supported`
  predictions on non-supporting gold cases
...
- `release_summary`: 面向 release gate 和 agent 的紧凑总览,包含 `status`、`next_action`、quality/sidecar gate 状态、关键 precision/recall/F1、false-support/abstention 风险计数、review queue top ids、acceptance blockers 和 label maturity；下游应优先读取它,只有需要诊断时再展开完整报告
- `false_support_analysis`: 汇总 total overcall count、`false_support_case_ids`、`weak_false_support_case_ids` 和 `high_risk_overcall_case_ids`,并按 case type、evidence scope、language 和 split 分组；每个分组都列出 `false_support_case_ids` 与 `weak_false_support_case_ids`,同时提供 `acceptance_guard`、`risk_slices` / `top_risk_slice` 和 `false_support_analysis.review_plan`,把 contradicted overcalls、hard negatives、full-text boundary、test split 和 non-English overcalls 排成机器可读的复核优先级,用于发布前优先复核最危险的 supported 误报；`review_plan.status` 会区分 `clear`、`review_required` 和 `blocked`,并用 `supported_overcall_blockers`、`weak_support_overcall_review`、`highest_risk_slice_review` 三个 phase 指导 agent 先处理阻断项。每个 phase 都带 `annotation_packet` / `command_template`,顶层 `recommended_annotation_packets` 和 `recommended_annotation_case_ids` 可以直接生成 blinded annotation packet,但这些 packet 是复核任务,不是自动改 label 或放行 supported 的许可。
- `acceptance_guard`: 顶层和 `false_support_analysis.acceptance_guard` 都会暴露 `ok_to_accept_supported`、`block_acceptance_case_ids` 和 `review_before_accepting_case_ids`; `supported` overcall 阻断接受/发布,`weakly_supported` overcall 必须人工复核后才能当作支撑使用
- `abstention_analysis`: 汇总 `incorrect_abstention_count`、
  `correct_abstention_count`、`review_case_ids`,并按 case type、evidence
  scope、language 和 split 分组,用于区分正确保守拒答与需要召回/矛盾复核的
...
`false_support_analysis.top_risk_slice`、`abstention_analysis.review_case_ids`、
`acceptance_guard.ok_to_accept_supported`、`quality_gate.review_queue_case_ids` 和
`quality_gate.critical_review_case_ids`,适合 agent 或发布脚本只读取最高风险
case 队列、supported-overcall 优先级和 abstention 召回损失。
...
`false_support_risk_slice_count`、`false_support_ok_to_accept_supported`、
`false_support_block_acceptance_count`、`false_support_block_acceptance_case_ids`、
`false_support_review_before_accepting_case_ids`、`false_support_top_risk_slice_id` 和
`false_support_top_risk_slice_case_ids`;baseline comparison artifact 还会保留
`support_baseline_metric_fields`、`support_baseline_metrics`、
`false_support_overcall_backends` 和 `false_support_top_overcall_backend`,让
`false_support_top_overcall_review_plan_status`、
`false_support_top_overcall_review_plan_next_action` 和
`false_support_top_overcall_review_plan_phase_ids`,让 agent 或 release
reviewer 可以不展开完整 `result.json` 就同时比较 macro/weighted F1、
abstention、false-support 风险,并发现哪次实验需要优先复核
supported-overcall 风险。默认不写
...
macro/weighted precision/recall/F1、abstention rate、false-support rate、
support overcall count/rate、contradiction recall、error bucket
counts、`total_overcall_count`、high-risk false support case ids、`false_support_risk_slices` / `top_false_support_risk_slice` 和分组级
`false_support_case_ids` / `weak_false_support_case_ids`,以及 acceptance
guard 字段 `block_acceptance_case_ids` / `review_before_accepting_case_ids`、
...
validation 还输出 `sidecar_case_provenance`,确认每条 sidecar row 都复制了
`label_source`、`case_type`、`evidence_scope`、`split` 和 `lang`,并通过
`missing_count` / `missing_case_ids` 直接暴露 sidecar 是否漏掉 dataset case。
...
`high_risk_reviewed_by_language` 和 `high_risk_unreviewed_by_language`,以及
`high_risk_case_count_by_language_case_type`、
`high_risk_reviewed_by_language_case_type` 和
`high_risk_unreviewed_by_language_case_type`,便于 agent 不展开 sidecar summary
也能按语言和 case type 分配 contradiction、hard-negative 或 full-text-boundary
复核缺口。当 `scripts/eval_support.py`
写出 experiment artifact 时,`manifest.json` 的 `result_summary` 也会保留
`support_label_dual_annotated`、`support_label_raw_dual_agreement_rate`、
`support_label_unresolved_disagreements`、`support_label_supported_disagreements`
和对应 case id 列表,并保留
`support_label_high_risk_case_count_by_language_case_type`、
`support_label_high_risk_reviewed_by_language_case_type` 与
`support_label_high_risk_unreviewed_by_language_case_type`,以及
`support_label_sidecar_provenance_missing_count` 和
`support_label_sidecar_provenance_missing_case_ids`,使发布工件本身能说明
标注成熟度、待 adjudication 风险和按语言/case type 的复核缺口。
validation 还会检查 status consistency:
...
规则、`review_protocol` 和不可修改字段,但不暴露隐藏 gold/adjudication 字段。标注员必须填写
...
split 统计的未审阅样本,完整 `unreviewed` 列表,以及包含 contradiction、
hard_negative、full_text_required 和 contradiction_set 的
`high_risk_unreviewed` / `high_risk_unreviewed_count` /
`high_risk_unreviewed_by_language` /
`high_risk_unreviewed_by_language_case_type`。优先审阅 contradiction、hard_negative、
full_text_required 和 contradiction_set 样本,因为这些最能暴露 false support、
过度支撑、contradiction-set aggregation 和 abstract/full-text 边界问题。
`full_text_required_unreviewed` 单独列出需要检查 lawful full-text evidence
是否真的存在的 abstract/full-text boundary cases,避免把 abstract-level
insufficient evidence 当成最终 full-text 结论。
`policy_boundary_unreviewed` 单独列出 medium-priority 但安全敏感的
weak_set_boundary citation-set cases,避免多篇弱证据被误升级为 full support。
`recommended_packets` 会给出机器可读的下一批 annotation-packet 命令,覆盖
balanced high-risk first review、按语言 high-risk review、full-text-boundary
first review、policy-boundary first review,以及按语言和 case type 精确拆分的
high-risk slice packet;存在 `single_annotator` case 时还会给出二审 packet。
`review_plan.phases[first_review_high_risk].candidate_case_count_by_language_case_type`
会重复高风险未审阅 cross table,便于按语言和风险类别拆分首轮标注。
...
需要阻断 abstract/full-text 或 weak citation-set 边界缺口时,分别使用
`--fail-on-full-text-required-unreviewed` 和
`--fail-on-policy-boundary-unreviewed`;audit gate 会列出对应 case ids,避免在
边界审阅前宣称 full-text 或 multi-citation support readiness。
sidecar validation 和 release gate 还会汇总 `label_source_counts`、
`reviewed_by_label_source`、`unreviewed_by_label_source`、
`reviewed_source_locator_count` 和
`published_benchmark_source_locator_count`,把 synthetic seed、人工审阅状态和
source locator 覆盖分开记录,避免把未审阅 fixture 误包装成成熟 benchmark。
...
`packet_id`、`review_protocol` 和 `packet_summary`,其中
`review_protocol` 明确 `packet_role`、`independent_labeling_required`、
`packet_target_annotator_count`、`benchmark_target_annotator_count`、
`second_review_required_after_first_review` 和
`adjudication_required_on_disagreement`;这些字段是 reviewer-assignment
protocol,不是 label evidence。`packet_summary` 记录 case ids 以及按语言、case type、
