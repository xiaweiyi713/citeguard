...
- `dataset`: 样本总数、case type 覆盖、evidence scope 覆盖、gold label 覆盖、split 覆盖和 label source 列表
...
`--audit` 输出 coverage、human-reviewed 数量、按 case type / split 统计的
未审阅样本,完整 `unreviewed` 列表,以及只包含 contradiction、
hard_negative 和 full_text_required 的 `high_risk_unreviewed` /
`high_risk_unreviewed_count`。优先审阅
...
和 `--case-type` 也可重复使用。需要给标注员分配更小的确定批次时,
可以重复传 `--case-id` 点名稳定 case id,并用 `--limit` 截取过滤后的前
N 条:
