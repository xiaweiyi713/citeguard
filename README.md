...
最快上手:安装后先跑 `citeguard status` 看本地配置;用 `citeguard verify --title "..." --year 2024`、`citeguard audit citations.json`、`citeguard support --claim "..." --title "..."`、`citeguard support-set citations.json --claim "..."` 或 `citeguard support-audit claim_citations.json` 做命令行核验。接 MCP 时安装 `citeguard[mcp]` 或在源码签出中运行 `python -m pip install -e ".[mcp]"`(需 Python >= 3.10),然后启动 `citeguard-mcp`,在 MCP 客户端里配置 `"command": "citeguard-mcp"`。源码签出场景还可以运行 `python3 scripts/demo_verify.py` 看实时效果;已安装包场景请优先使用 `citeguard` / `citeguard-mcp` 入口。核心库与测试在 Python >= 3.9 下零依赖运行。
