    ...
    class FastMCP:  # type: ignore[no-redef]
        ...
        def run(self) -> None:
            raise RuntimeError(
                ...
                'Install with `python -m pip install -e ".[mcp]"` using Python 3.10 or newer.'
            )
else:
...
def verify_citation_tool(
    ...
    full_text: Optional[List[str]] = None,
    doi: str = "",
    arxiv_id: str = "",
) -> dict:
...
def check_claim_support_set_tool(
    ...
    The result aggregates per-citation abstract-level support checks. A supported
    aggregate requires at least one resolved citation to strongly support the
    claim; contradictions are surfaced as high-risk.
    """
    if not str(claim).strip():
        return error_payload(
            "missing_claim",
...
def search_counterevidence_tool(claim: str, top_k: int = 5) -> dict:
    ...
    return search_counterevidence_candidates(claim, _source(), top_k=parsed_top_k).to_dict()
...
def audit_claim_support_tool(
    ...
    for index, item in enumerate(items, start=1):
        ...
        requests.append(
            ClaimSupportAuditItem(
                claim=claim,
                citations=[_parse_citation_item(item, tool="audit_claim_support_tool", index=index)],
                lang=str(item.get("lang", "")),
                input_mode="citation",
            )
        )
...
def _normalize_evidence_chunks(item: Dict[str, Any]) -> list:
    chunks = []
    for index, value in enumerate(_as_list(item.get("evidence_chunks")), start=1):
        if isinstance(value, dict):
            chunks.append(dict(value))
        elif str(value).strip():
            chunks.append(_chunk(str(value), f"user_evidence_chunk_{index}", "metadata"))

    for index, value in enumerate(_as_list(item.get("evidence_text")), start=1):
        if isinstance(value, dict):
            chunk = dict(value)
            chunk.setdefault("source_field", f"user_evidence_text_{index}")
            chunk.setdefault("evidence_scope", "metadata")
            chunks.append(chunk)
        elif str(value).strip():
            chunks.append(_chunk(str(value), f"user_evidence_text_{index}", "metadata"))

    for index, value in enumerate(_as_list(item.get("full_text")), start=1):
        if isinstance(value, dict):
            chunk = dict(value)
